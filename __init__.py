...
class Stock():
    ...
    def getSt(self):#获取报告日期的上市公司的股价
        Firstday=datetime.datetime(EndDate.year,EndDate.month,1)#获取报告日同月第一天
        Firstday=Firstday.strftime('%Y%m%d')
        sql="SELECT closeprice FROM stockprice WHERE secid='%s' and tradedate >='%s' and tradedate<='%s'"%(secid,Firstday,EndDate)
        df=pd.read_sql(sql,conn)#将查询结果转化为dataframe格式
        data=df['closeprice'].tolist()#将dataframe转为list
        St=data[-1]#获取股价为当月有效数据最后一天的数据
        return St
